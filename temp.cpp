...
void input()
{
	...
	for (i = 1; i <= n; i++)
	{
		...
		{ /*这用于判断该学生姓名是否为y*/
			x = getchar();
			if (x == 9 || x == ' ')
				strcpy(inform[i].name, s); // 这里判断输入的这个y后面的是tab或空格
			if (x == '\n')
			{
				flag++;
				goto w;
